 ...
 */
	public function link($title, $url = array(), $options = array()) {
		if(empty($url) and empty($options)){ 
			return $this->Html->link($title, 'javascript:void(0);', array('escape'=>false));
		}
 ...
 */
	protected function _pagingLink($which, $title = null, $options = array(), $disabledTitle = null, $disabledOptions = array()) {
		...
		if ($disabledTag) {
			...
			return $this->Html->tag($tag, '<a href="javascript:void(0)">'. $title .'</a>', compact('class'));
		}
		//return $this->Html->tag($tag, $title, compact('escape', 'class') + $options);
		return $this->Html->tag($tag, '<a href="javascript:void(0)">'. $title .'</a>', compact('escape', 'class') + $options);
	}
 ...
 */
	public function numbers($options = array()) {
		...
		if ($modulus && $params['pageCount'] > $modulus) {
			...
			if ($currentTag) {
				$out .= $this->Html->tag($tag, $this->Html->tag('<a href="">'. $currentTag ."</a>", $params['page']), array('class' => $currentClass));
			} else {

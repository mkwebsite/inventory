<?php
	...
	class UploadComponent extends Component {
				...
				if(!in_array(strtolower($this->ext($fileName)),$this->_allowed)){
					...
					if (is_uploaded_file($fileTmp)) {
						...
						if ($rules == NULL) {
							...
							$output = $fileName;
							// -- just upload it
							if (copy_uploaded_file($fileTmp,$output)) {
								chmod($output, 0755);
								$this->result = basename(strtolower($this->_name));
							} else {
							...
							if (function_exists("imagecreatefromjpeg")) {
								...
								if (!isset($rules['quality'])) $rules['quality'] = 75;
		...
		function delete($img, $destination) {
		...
		function image ($file, $type, $size, $output = NULL, $quality = NULL) {
			...
			switch ($type) {
				...
				case 'resize':
					...
					if ($uploadWidth > $maxScale || $uploadHeight > $maxScale) { 
							$newX = $maxScale ;
							$newY = ($uploadHeight/$uploadWidth)*$newX ; 
					} else {
						$newX = $uploadWidth;
						$newY = $uploadHeight;
					}
